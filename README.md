...
- [docs/architecture.md](docs/architecture.md) — How the tool is put together: its main parts, how a ROMs folder's data flows into the registry and back out — a whole folder at a time or one chosen game, filling its gaps or replacing what it holds — how the registry is browsed, corrected and pruned, how the two long operations started from the browser run in the background alongside the served pages, and how a hand-made correction — or a whole protected game — is kept from being overwritten.
