...
- Passes `cfg.RomsFolders` to `webui.Handler` alongside the registry folder (backlog item 023): the web UI's completion writes to those folders, and this is the one place already reading the configuration, so `internal/webui` never reads it itself. A completion running in the background is **not** waited for on shutdown — the 5s grace period covers in-flight requests only. That is deliberate rather than overlooked: `gamelist.WriteFile` swaps its file in atomically ([`decisions/026`](../decisions/026-a-gamelist-is-written-beside-then-swapped-in.md)), so a run cut short by the process exiting leaves the user's files intact, and waiting would have forced `webui.Handler` to return something other than an `http.Handler`.
