...
- Sending or completing a single game no longer stops at a folder whose `gamelist.xml` does not list it yet: if the ROM file is genuinely there and the registry already knows the game, a new entry is created for it, filled with everything the registry knows, instead of being refused. A folder that truly does not hold the ROM is still turned down exactly as before, and the confirmation says plainly that a new entry was created rather than claiming a completion or a replacement.
...
- Check one ROMs folder's own status from the browser, instead of the registry's view of everything at once: for each of its systems, how many ROM files are found, how many are already fully scraped locally, how many are missing something, and how many have no local entry at all — worst systems first. Every problem game is named, saying whether "Complete the ROMs folders" would already fix it from what the registry knows, or whether it needs real scraping — and links straight to that game's own page in the registry whenever it already knows it, to finish the scrape by hand right there. An `_info.txt` note left in a system's folder is never mistaken for a missing ROM, and a game EmulationStation itself marks hidden is left out of the report entirely. Opening the page changes nothing.
- Get detailed, command-specific help with `--help` on any command (e.g. `update --help`), instead of just the generic top-level help.
- Act on a single problem game straight from its ROMs folder's status page, instead of dropping to the command line or hunting for it in the registry. A game already scraped locally but unknown to the registry gets a one-click "Retrieve into the registry" button; a game the registry already knows gets a direct link to send it to that same folder, with the folder and the safe "fill the gaps only" rule already chosen.
<!-- vibe:end:features -->
...
The home page also lists each configured ROMs folder under "ROMs folders", leading to a status report of that one folder alone — useful because the registry's own view holds everything every folder ever scraped, which can make a folder look more complete than it really is. For each of its systems, the report counts the ROM files found, how many already have a fully scraped local entry, how many have one but it is missing something, and how many have no local entry at all; systems with the most gaps are listed first. Every problem ROM is then named, saying whether "Complete the ROMs folders" would already fix it — the registry holds what is missing — or whether it needs real scraping, since the registry does not have it either. A ROM with no local entry is spotted by looking at what sits directly in the system's folder, aside from `gamelist.xml`, an `_info.txt` note some setups leave there, and subfolders like `images`/`videos`; a stray file that is not really a ROM can therefore still be counted by mistake. A game EmulationStation itself marks hidden (`<hidden>true</hidden>` in the local `gamelist.xml`) is left out of the report entirely — not counted, not listed as missing — even though its ROM file is still there. Every problem ROM the registry already knows about links straight to that game's own page, so its scrape can be finished or corrected by hand without hunting for it. Opening the page changes nothing, in the ROMs folder or in the registry, and a folder that is no longer configured, or that can no longer be read, is named with a clear message instead of a broken page.
<!-- vibe:end:usage -->
