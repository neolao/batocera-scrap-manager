...
- On success, prints a one-line confirmation (`"removed <rom-filename> (system: <system>)"`), then persists through [`modules/store.md`](store.md)'s `Save` so the consultation site stops listing the removed game (backlog item 019), and returns exit code 0.
- That last step deliberately bypasses `saveAndGenerateSite`, the helper every other mutating command uses: its "print `error:`, fail the command" semantics assume nothing was applied, which is never true after `registry.Remove` erased the game's file. Every `store.Save` failure is therefore an extra `warning:` line, never a non-zero exit — see [`decisions/023`](../decisions/023-remove-warns-about-persistence-it-never-denies-the-deletion.md), which extends [`decisions/022`](../decisions/022-a-deletion-is-committed-when-the-game-file-is-gone.md) to the CLI.
- `persistenceWarning` words the two failures apart, since their recovery differs: an error wrapping `store.ErrSiteNotRegenerated` names `update` as the way to rebuild the stale site, while any other error reports that the registry itself could not be fully written and that *other* entries may be unsaved — sending that case to `update` would disguise it as a display problem.
- A failed removal never reaches `store.Save`, so `ErrGameNotFound` leaves the registry folder — `index.html` included — exactly as it was.
...
- Passes `cfg.RomsFolders` to `webui.Handler` alongside the registry folder (backlog item 023): the web UI's completion writes to those folders, and this is the one place already reading the configuration, so `internal/webui` never reads it itself. A completion running in the background is **not** waited for on shutdown — the 5s grace period covers in-flight requests only. That is deliberate rather than overlooked: `gamelist.UpdateFile` swaps its file in atomically ([`decisions/026`](../decisions/026-a-gamelist-is-written-beside-then-swapped-in.md)), so a run cut short by the process exiting leaves the user's files intact, and waiting would have forced `webui.Handler` to return something other than an `http.Handler`.
