...
## Web UI
**Definition:** The registry served live over HTTP by the `serve` command: a page listing every game grouped by system, and one page per game — addressed by its Game ID — showing its full metadata and every medium available for it, the form correcting that metadata, and the control declaring the game right as a whole (see Protected game). Deleting a game and managing its media are still to be built on the same per-game URLs.
