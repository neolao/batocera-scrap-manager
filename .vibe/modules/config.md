# Module: config
**Role:** Persists user configuration — the registry folder and the Batocera ROMs folders to watch.
**Files:** `internal/config/config.go`
**Exports:**
- `config.Config{RegistryFolder, RomsFolders}`
...
- `(*Config).SetRegistryFolder(path string) error` — converts to an absolute path
- `(*Config).AddRomsFolder(path string) (added bool, err error)` — deduplicates by absolute path

**Depends on:** (no internal module)

Stored as JSON (`registry_folder`, `roms_folders`).
